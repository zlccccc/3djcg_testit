...
def parse_predictions(end_points, config_dict, nms_soft_sem_score=True):
    ...
    pred_heading = end_points['pred_heading']  # (B, N)
    pred_center = end_points['pred_center']
    pred_box_size = end_points['pred_size']
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            heading_angle = pred_heading[i, j].detach().cpu().numpy()
            box_size = pred_box_size[i, j].detach().cpu().numpy()
    ...
    if config_dict['remove_empty_box']:
        ...
        for i in range(bsize):
            ...
            for j in range(K):
                ...
                if np.max(np.max(box3d, axis=0)-np.min(box3d, axis=0)) <= 1e-4:
    ...
    if not config_dict['use_3d_nms']:
        ...
        for i in range(bsize):
            ...
            for j in range(K):
                ...
                boxes_3d_with_prob[j,4] = np.max(pred_corners_3d_upright_camera[i,j,:,1])
                boxes_3d_with_prob[j,5] = np.max(pred_corners_3d_upright_camera[i,j,:,2])
                if nms_soft_sem_score:
                    boxes_3d_with_prob[j,6] = obj_prob[i,j] * pred_sem_cls_prob[i,j]
                else:
                    boxes_3d_with_prob[j,6] = obj_prob[i,j]
